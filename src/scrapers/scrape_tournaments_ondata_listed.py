    ...
    for table in tables:
        for row in table.find_all("tr")[1:]:
            ...
            if full_url and ondata_id:
                longname = _fetch_tournament_longname(ondata_id, logger, logger_keys)
            else:
                longname = None
            ...
            if action: 
                logger.info(f"Tournament {raw.shortname} on {raw.startdate} successfully {action}")
                logger.success(logger_keys, f"Tournament successfully {action}")

    logger.summarize()


def _fetch_tournament_longname(ondata_id: str, logger, logger_keys) -> Optional[str]:
    """
    ...
    try:
        ...
        logger.failed(logger_keys, f"Error fetching longname: {e}")
        return None