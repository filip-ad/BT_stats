...
# ==================================================================================================
SCRAPE_PARTICIPANTS_CLASS_ID_EXTS = ['110']  
...
def scrape_tournament_class_knockout_matches_ondata(cursor, run_id=None):
    ...
    for idx, tclass in enumerate(classes, 1):
        ...
        try:
            ...
            if is_two_page_split_128:
                ...
                for page_words in pdf_pages_words[:2]:
                    ...
                    if rounds_half:
                        ...
                        _apply_walkovers_to_rounds(rounds_half, markers_page, scores_hint=scores_half)
            ...
            if not split_built:
                _apply_walkovers_to_rounds(all_rounds, wo_markers_all, scores_hint=all_scores_page)
...
def _extract_winner_entries(words: Sequence[dict], x_range: Tuple[float, float]) -> List[WinnerEntry]:
    ...
    for word in words:
        ...
        if any(ch.isdigit() for ch in label):
            continue
        if " " not in label and not player_id_ext:
            continue
...
def _allocate_winners_by_round(
    ...
    for expected in _expected_winner_counts(tree_size):
        ...
        while len(current) < expected and band_idx < len(band_entries):
            ...
            if len(entries) >= expected and winners_by_round:
                targets: List[float] = []
    ...
    *,
    ...
    for w in wo_words:
        ...
        for ridx, match, bounds in flat_matches:
            if not match.players:
                continue
            if abs(match.center - y) > y_tolerance:
                continue
            if scores_hint and pid is None and match.scores is None:
                left, right = bounds
                nearby_scores = 0
                for entry in scores_hint:
                    if abs(entry.center - match.center) > y_tolerance:
                        continue
                    ...
                    nearby_scores += 1
                if nearby_scores >= 2:
                    skip_due_to_scores = True
                    break
            if match.scores is not None and not (override_scored and pid):
                # Keep existing scores unless the WO marker explicitly names a player id.
                continue
            left, right = bounds
            if left is not None and right is not None:
...
def _apply_walkovers_to_rounds(
    ...
    *,
    ...
    if not wo_markers:
        return
    markers = list(wo_markers)
    for matches in rounds:
        for match in matches:
            if match.scores is not None:
                # If we already have scores, don't override with WO
                continue
            if not match.players:
                continue
            bounds = _walkover_bounds_for_match(match)
            if scores_hint:
                left, right = bounds
                nearby_scores = 0
                for entry in scores_hint:
                ...
                if nearby_scores >= 2:
                    continue
            if _is_walkover(match.center, markers, match_bounds=bounds, y_tolerance=y_tolerance, x_tolerance=x_tolerance):
                match.walkover = True
                match.scores = None
                if match.winner is None and match.players:
                    match.winner = match.players[0]
                if match.winner and len(match.players) == 2:
