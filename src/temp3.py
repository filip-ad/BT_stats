...
DEFAULT_PDF_URL = "https://resultat.ondata.se/ViewClassPDF.php?classID=30159&stage=5"
...
def build_quarterfinals(
    ...
    # Winners deduced from the bracket structure (see analysis in docstring).
    winner_indices = [0, 3, 5, 6]
    matches: List[Match] = []
    semifinal_players: Optional[List[Player]] = None
    ...
    for idx in range(4):
        first = r16_matches[2 * idx].winner
        second = r16_matches[2 * idx + 1].winner
        assert first is not None and second is not None
        participants = [first, second]
        center = (r16_matches[2 * idx].center + r16_matches[2 * idx + 1].center) / 2
        match_scores = assign_nearest_score(center, remaining_scores)
        winner = r16_matches[winner_indices[idx]].winner
        if semifinal_players:
...
def build_semifinals(
    ...
    matches: List[Match] = []
    for idx in range(2):
        first = quarterfinals[2 * idx].winner
        second = quarterfinals[2 * idx + 1].winner
        assert first is not None and second is not None
        participants = [first, second]
