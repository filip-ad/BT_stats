...
def scrape_tournament_class_knockout_matches_ondata(cursor, run_id=None):
    ...
    for idx, tclass in enumerate(classes, 1):
        ...
        header_line = f"===== {tclass.shortname or tclass.longname or 'N/A'} [ext={cid_ext or 'None'}] ====="
        if DEBUG_OUTPUT:
            _debug_print("\n" + header_line)
        ...
        try:
            ...
            logger.info(
                logger_keys.copy(),
                f"Removed: {removed_total}   Inserted: {class_inserted}   Skipped: {class_skipped}",
            )
            ...
            if stored is None:
                ...
                logger.success(logger_keys.copy(), f"SELF-CHECK: parsed={parsed_size} == stored={stored}")
