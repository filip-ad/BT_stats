...
def upd_participants(scrape_ondata=False, resolve=False):
    """
    Orchestrate the update of participant data by optionally scraping raw data and resolving to clean tables.
    Args:
        scrape_ondata: If True, scrape participant data (including seeds and final positions) from OnData PDFs.
        resolve: If True, resolve raw data into clean participant and participant_player tables.
    """
    conn, cursor = get_conn()

    try:
        # Scrape participant data from OnData
        if scrape_ondata:
            scrape_participants_ondata(cursor)
        ...
        logging.error(f"Error in upd_participants: {e}", stack_info=True, stacklevel=3, exc_info=True)
