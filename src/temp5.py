...
def build_round_of_16(
    ...
    for winner_entry in winners:
        ...
        participants.sort(key=lambda player: abs(player.center - winner_entry.center))
        ...
        try:
            ...
            pass
        score_entry = assign_nearest_score(winner_entry.center, remaining_scores)
        match_scores = score_entry.scores if score_entry else None
        if winner is None and score_entry and score_entry.inline_winner:
            try:
                winner = match_short_to_full(
                    score_entry.inline_winner.short,
                    score_entry.inline_winner.center,
                    players,
                    score_entry.inline_winner.player_id_ext,
                )
            except ValueError:
                pass
...
def build_round_from_previous(
    ...
    winners: Optional[Sequence[WinnerEntry]],
    scores: List[ScoreEntry],
    players: Sequence[Player],
    winner_tolerance: float = 18.0,
    score_tolerance: float = 20.0,
    copy_winners: bool = True,
) -> List[Match]:
    remaining_scores = scores.copy()
    if winners is None:
        remaining_winners: List[WinnerEntry] = []
    elif copy_winners:
    ...
    while idx < len(previous_round):
        ...
        if first.winner:
            ...
            participants.append(first.players[0])
        if second and second.winner:
            participants.append(second.winner)
        elif second and second.players:
            participants.append(second.players[0])
        ...
        score_entry = assign_nearest_score(center, remaining_scores, score_tolerance)
        winner = None
        if nearest_winner:
            try:
                winner = match_short_to_full(
...
def fill_missing_winners(source_round, target_round):
    """If a source match lacks a winner, infer it from participants in the next round."""
    advancing_players = [player for match in target_round for player in match.players]
    for src in source_round:
        if src.winner is not None:
            continue
        candidates = [player for player in src.players if player in advancing_players]
        if len(candidates) == 1:
            src.winner = candidates[0]
...
@@ -364,78 +435,107 @@ def parse_args() -> argparse.Namespace:
     parser.add_argument(
         "url",
         nargs="?",
...
+        r32_winners = extract_winner_entries(words, (300, 420))
+        r32_scores = extract_score_entries(words, (260, 380))
...
+        r16_progress_winners = extract_winner_entries(words, (380, 430))
+        r16_progress_scores = extract_score_entries(words, (420, 520))
...
+    if r64_matches:
+        qf_winner_pool = list(extract_winner_entries(words, (460, 520)))
+        qf_matches = build_round_from_previous(
+            r16_matches,
+            qf_winner_pool,
+            qf_scores,
+            players,
+            winner_tolerance=25.0,
+            score_tolerance=25.0,
+            copy_winners=False,
+        )
+        sf_winner_pool = qf_winner_pool
+    else:
+        sf_winner_entries = extract_winner_entries(words, (380, 420)) or extract_winner_entries(words, (400, 470))
+        sf_winner_pool = list(sf_winner_entries) if sf_winner_entries else []
+        qf_matches = build_quarterfinals(r16_matches, qf_scores, players, sf_winner_pool)
...
+    if r64_matches:
+        semifinals = build_round_from_previous(
+            qf_matches,
+            sf_winner_pool,
+            sf_scores,
+            players,
+            winner_tolerance=30.0,
+            score_tolerance=30.0,
+            copy_winners=False,
+        )
+    else:
+        semifinals = build_semifinals(qf_matches, sf_scores, sf_winner_pool, players)
...
+    if r64_matches and sf_winner_pool:
+        final_entry = sf_winner_pool[0]
+    else:
+        final_winner_list = extract_winner_entries(words, (460, 520)) or extract_winner_entries(words, (520, 600))
+        final_entry = final_winner_list[0]
+    final_match = build_final(semifinals, final_scores, final_entry, players)
